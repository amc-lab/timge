...
def _liftover_bed(infile: StringIO, lifter: Lifter, genome: str) -> [str, StringIO]:
    """
...
def _liftover_bedgraph(
    infile: StringIO, lifter: Lifter, genome: str
) -> [str, StringIO]:
    """
    PRIVATE. Liftover for BED* format data
...
def _liftover_dotplot(infile: StringIO, lifter: Lifter, genome: str) -> [str, StringIO]:
    ...
    for line in infile:
        ...
        if 0.1 <= prob < 0.3:
            prob_pairs[0].append(
                (lifter(genome, seqid, left)[1], lifter(genome, seqid, right)[1])
            )
        elif 0.3 <= prob < 0.8:
            prob_pairs[1].append(
                (lifter(genome, seqid, left)[1], lifter(genome, seqid, right)[1])
    ...
    prob_pairs = [sorted(pairs, key=lambda x: (x[0], -x[1])) for pairs in prob_pairs]
    ref_seqid = lifter(genome, seqid, 0)[0]
    outfile = StringIO()
    print(
...
def annotate(
    infile: StringIO, lifter: Lifter, seqs: dict, genome: str
) -> [str, StringIO]:
    """ """
            ...
            if points:
                ...
                for p in points:
                    if reverse:
                        ...
                        if step == 3:
                            p_site = (
                                Seq(seqs[seqid].seq[p_start:p_stop])
                                .reverse_complement()
                                .translate()
                            )
                        else:
                            p_site = Seq(
                                seqs[seqid].seq[p_start:p_stop]
                            ).reverse_complement()
                    else:
