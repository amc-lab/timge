...
IUPAC_AMBIG_TO_NTS.update({"A": "A", "C": "C", "G": "G", "T": "T", "U": "T"})
...
def consensus_base(column: str, iupac: bool = False) -> str:
    ...
    for base, count in Counter(column).items():
        for nt in IUPAC_AMBIG_TO_NTS[base]:
            nts[nt] += count
    nts = sorted([(k, v) for k, v in nts.items()], key=lambda x: x[1], reverse=True)
    if not iupac:
    ...
    if len(nts) == 1:
        return nts[0][0]  # Return majority
    return IUPAC_NTS_TO_AMBIG[
        "".join(sorted([nt[0] for nt in nts]))
    ]  # Return iupac indecision
...
def test_aligners() -> set[str]:
    ...
    return sorted(aln for aln in aligners)


def align(
    file: StringIO, prog: str = "mafft", threads: int = 1
) -> tuple[int, StringIO]:
    """ """
    if prog == "mafft":
        with Popen(
    ...
    if prog == "kalign":
        with (
            Popen(["kalign"], stdin=PIPE, stdout=PIPE, stderr=PIPE, text=True) as P,
            Popen(
                shlex.split("sed -n '/^>/,$p'"), stdin=P.stdout, stdout=PIPE, text=True
            ) as P2,
        ):
